...
prompt('Aperte Enter para finalizar.')// Module import
const prompt = require('prompt-sync')();
...
function getTriangleSides () {
  ...
  for (let i = 0; i < 3; i++) {
    while (true) {
      ...
      if (validateValue(triangleSideStr)) {
        triangleSides.push(+triangleSideStr.trim().replace(',', '.'));
        console.log('');
        break;
      } else {
        console.log('Valor inválido, tente novamente.\n');
...
function getTriangleType (sidesArray) {
...
if (valuesFormTriangle) {
  const triangleType = getTriangleType(triangleSides);

  console.log(`Os segmentos acima PODEM formar um triângulo ${triangleType}!\n`);
} else {
  console.log('Os segmentos acima NÃO PODEM formar um triângulo!\n');
};
